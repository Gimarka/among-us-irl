...
//
// The body/head artwork below is traced from a supplied illustration (a
// pair of flexed, muscular arms framing an oval face) rather than
// hand-drawn like the rest of this file - hence the odd coordinate scale:
// it's defined in the original trace's units (roughly 0-6000 x 0-3350) and
// mapped into this SVG's viewBox by BODY_TRANSFORM, the same way the
// source file itself did with its own <g transform>.
//
// The traced artwork is pure line art - every shape is a thin closed
// outline, not a filled silhouette, so simply colouring it only colours
// those thin lines and leaves the body hollow. To get a genuinely filled,
// coloured body with a crisp black outline on top (the actual look we
// want), the arms are drawn twice: once through a "dilate" filter that
// fattens the coloured lines until the gaps between them close up into a
// solid fill, and again on top in plain black for the crisp linework. A
// mask keeps that fattening from ever bleeding into the visor's hole.
//
// Every gradient/clip/filter/mask id is prefixed because the markup is
// used more than once on the page, and duplicate ids would make one
// instance reference another's definitions.
...
// exactly like the source SVG's <g transform="translate(0,335)
// scale(0.1,-0.1)"> did.
const BODY_TRANSFORM = 'translate(0,335) scale(0.1,-0.1)';

// The dilate radius is specified in the artwork's own (pre-BODY_TRANSFORM)
// units, same as the path data - so it looks tiny here but is a real ~28
// units once BODY_TRANSFORM's 0.1 scale is applied to it too.
const BODY_FILL_DILATE_RADIUS = 280;
...
// its bounds left a visible gap all the way round), used both as the
// selfie's clip and as the mask that protects it from BODY_FILL_DILATE.
  ...
  return `
    <svg class="character-svg" viewBox="${VIEW_BOX}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs>
        ...
        <filter id="${id}-bodyFillDilate" x="-60%" y="-60%" width="220%" height="220%">
          <feMorphology operator="dilate" radius="${BODY_FILL_DILATE_RADIUS}" in="SourceGraphic"/>
        </filter>

        <!-- Keeps the dilated fill below from ever bleeding into the
             visor's hole, however far BODY_FILL_DILATE_RADIUS spreads it. -->
        <mask id="${id}-bodyFillMask" maskUnits="userSpaceOnUse" x="-5" y="-45" width="630" height="380">
          <rect x="-5" y="-45" width="630" height="380" fill="#fff"/>
          <path transform="${BODY_TRANSFORM}" d="${VISOR_HOLE_PATH}" fill="#000"/>
        </mask>
      </defs>

      <!-- Glass visor, shown until there is a selfie to put behind it. Drawn
           first (and the photo right after it) so the body/head silhouette
           painted on top naturally masks everything outside the ring's own
           hole - no separate clip needed for that outer edge. -->
      ...
      <!-- The coloured "fill" pass: the arms fattened by the dilate filter
           until their thin line-art gaps close into a solid body, masked
           so that fattening can't creep into the visor's hole. -->
      <g mask="url(#${id}-bodyFillMask)">
        <g class="character-suit" filter="url(#${id}-bodyFillDilate)" transform="${BODY_TRANSFORM}">
