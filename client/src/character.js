...
// mapped into this SVG's 600x335 viewBox by BODY_TRANSFORM, the same way
// the source file itself did with its own <g transform>.
//
...
// Maps the traced artwork's own coordinate space onto the 600x335 viewBox
// below, exactly like the source SVG's <g transform="translate(0,335)
// scale(0.1,-0.1)"> did.
const BODY_TRANSFORM = 'translate(0,335) scale(0.1,-0.1)';

// The face sits inside the oval hole of the head/visor ring path itself
// (BODY_RING below) - there's no separate helmet shape to cut a hole in,
// the ring's own two opposite-wound subpaths already form a torus. These
// bounds were measured directly off that path (its inner subpath's own
// bounding box) after applying BODY_TRANSFORM.
const VISOR = 'cx="308.5" cy="172" rx="73.8" ry="120.8"';
...
export function characterMarkup(id) {
  ...
  return `
    <svg class="character-svg" viewBox="0 0 600 335" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs>
      ...
      <!-- Glass visor, shown until there is a selfie to put behind it. Drawn
           ...
           oval hole - no separate clip needed for that outer edge. -->
      ...
      <!-- Arms and the head/visor ring - the recolourable "suit". The ring
           is a single path with two opposite-wound subpaths (an outer
           outline and the inner oval), which is what leaves its centre
           transparent for the photo above to show through. -->
      ...
      <!-- Shading details (bicep and chin creases) - always dark, like the
           fixed outline strokes on the hand-drawn hats, regardless of suit
           colour. -->
