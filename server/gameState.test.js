...
test('addPlayer adds a player and returns the full list', () => {
  ...
  assert.deepEqual(result, [{ id: 'socket1', name: 'Alice', photo: null, color: null, hat: null }]);
});
...
test('removePlayer takes a player out of the list', () => {
  ...
  assert.deepEqual(result, [{ id: 'socket2', name: 'Bob', photo: null, color: null, hat: null }]);
});
